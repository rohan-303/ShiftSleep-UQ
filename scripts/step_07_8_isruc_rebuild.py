...
def main():
    ...
    for n in EXPECTED:
        ...
        acquired_ok = bool(rec and s1 and s1x and s2 and s2x and ra.get("body_valid") and ra.get("byte_delta") == 0)
        subj_rows.append({"subject_id":sid,"expected_recording_count":1,"acquired":str(acquired_ok).upper(),"included":str(status=="INCLUDED").upper(),"subject_status":"COMPLETE" if status=="INCLUDED" else status,"montage_variant":variant,"valid_epochs":valid,"wake":counts["wake"],"n1":counts["n1"],"n2":counts["n2"],"n3":counts["n3"],"rem":counts["rem"],"failure_reason":reason,"data_contract_version":CONTRACT,"preprocessing_version":PREPROCESSING})
