...
def words(p):
 s=Path(p).read_text(encoding='utf-8');s=re.sub(r'%.*','',s);s=re.sub(r'\\(citep|ref|label|texttt|textbf|textit)\{[^}]*\}',' ',s);s=re.sub(r'\\[A-Za-z]+',' ',s);s=re.sub(r'[^A-Za-z0-9]+',' ',s);return len(s.split())
render=[
('main','17','all pages','figures/tables/captions/references','PASS','Rendered build_main/main.pdf; page contact sheet inspected'),
('supplement','2','all pages','section flow/table/text','PASS','Rendered supplement.pdf; page contact sheet inspected')]
with open(R/'step19_render_audit_v1.csv','w',newline='',encoding='utf-8') as f:
 w=csv.DictWriter(f,fieldnames=['document','pages','scope','checks','status','evidence']);w.writeheader();w.writerows(dict(zip(w.fieldnames,x)) for x in render)
prov={'step':'19','status':'INTERNAL_REVIEW_PARTIAL','reason':'Revised main PDF is verified at paper/main_step19_candidate.pdf but paper/main.pdf is held open by another process and could not be atomically replaced during this run.', 'main_tex_sha256':sha(ROOT/'paper/main.tex'),'main_pdf_sha256_current':sha(ROOT/'paper/main.pdf'),'main_pdf_sha256_candidate':sha(ROOT/'paper/main_step19_candidate.pdf'),'main_pdf_candidate_path':'paper/main_step19_candidate.pdf','supplement_tex_sha256':sha(ROOT/'paper/supplement.tex'),'supplement_pdf_sha256':sha(ROOT/'paper/supplement.pdf'),'references_bib_sha256':sha(ROOT/'paper/references.bib'),'frozen_evidence_manifest_sha256':sha(R/'paper_evidence/paper_evidence_manifest_v1.json'),'b0_version':'v1_2','b1_gate':'B1_PRIMARY_EVALUATION_COMPLETE','step16_gate':'STEP16_METHOD_GATE_COMPLETE','method_gate':'RELIABILITY_METHOD_NOT_AUTHORIZED','evidence_gate':'BENCHMARK_SYNTHESIS_FROZEN'}
(R/'step19_manuscript_provenance_v1.json').write_text(json.dumps(prov,indent=2)+'\n',encoding='utf-8')
report='''# ShiftSleep-UQ Step 19 Internal Review and Manuscript Revision Report
...
The revised candidate main PDF is 17 pages. The separate supplement is 2 pages.
...
Created `reports/step19_render_audit_v1.csv`. The 17-page main candidate and 2-page supplement were rendered to PNG contact sheets and visually inspected. No clipping, blank page, table overflow, unreadable figure, or broken supplement page was observed.
...
'''
(R/'STEP_19_INTERNAL_REVIEW_AND_MANUSCRIPT_REVISION_REPORT.md').write_text(report,encoding='utf-8')
